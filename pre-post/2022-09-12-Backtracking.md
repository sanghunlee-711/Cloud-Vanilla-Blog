---
slug: 2022-09-12-Backtrack
title: BackTracking, DFS란?
author: Sanghun lee
...
관련 문제를 만날 때 마다 해결책도 안떠오르고 답답해서 한번 정리를 해보려한.

# 2. Backtracking

Backtracking은 답변이 될 요소들을 점진적으로 만들고 조건을 충족하지 않는 경우 이를 폐기 시키는 방식이다.

백트래킹에서는 3가지 컨셉이 중요하다

- Choose: 잠재적 답변 예제를 선택한다.
  ex) 잠재적 답변예제는 주어진문자열에서 생성될 수 있는 모든 하위 문자열.
- Constraint: 선택한 예시가 충족해야 하는 제약 조건을 정의합니다
  ex) 문자열이 회문이어야 한다.
- Goal: 필요한 솔루션을 찾았는지 여부를 결정하는 목표를 정의하고 역추적해야 합니다
  ex) 문자열의 끝에 도달하면 목표가 달성됩니다.
