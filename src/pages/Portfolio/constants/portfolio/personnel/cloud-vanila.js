export const CLOUD_VANILLA_PORTFOLIO = {
  ...
  devHighlights: [
    '주제별 게시글 및 댓글 작성',
    '모바일 대응을 위한 반응형 UI/UX',
    '이력서 및 포트폴리오 프린트',
  ],
  devAchivement: [
    'Vanila js를 통한 state기반 렌더링 방식의 구현',
    '다양한 라우팅 방식에 대한 이해',
    'Markdown의 파싱을 위한 라이브러리 활용법',
  ...
  devExplains: [
    ...
    },
    {
      ...
      explanation: `하나의 EC2서버에서 Nginx의 포트포워딩을 통해 백엔드와 프론트를 제공하고 있습니다. 
      규모가 크지 않은 프로젝트이므로 불필요한 낭비를 위해 내부 폴더를 DB처럼 활용하여 MarkDown파일들을 불러오며 프론트엔드에서 호출하는 API를 통해 파싱된 객체를 내려줍니다.`,
    },
  ],
};
