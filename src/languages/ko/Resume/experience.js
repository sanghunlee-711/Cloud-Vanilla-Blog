export const SOCAR = {
  ...
  raffleBackoffice: {
    ...
    delivery:
      "backoffice(marketing)에서 eventPage/winnerPage 폼 편집을 담당하고, socar-server-frontend의 /event/single-benefit-draw-preview를 <object>로 임베딩해 미리보기 구조를 분리",
    impact:
      "실서비스(/event/single-benefit-draw/:eventId)는 API 기반, 미리보기는 ?previewdata(JSON) 파싱 기반으로 데이터 소스를 분리해 object data URL 갱신만으로 빠른 검수/운영이 가능하도록 개선",
  },
  couponWebview: {
